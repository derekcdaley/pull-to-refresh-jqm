var pullToRefresh ={
	onRefresh: function(){console.log('No defined refresh function!');},
	refreshPTRContent: function(){
		setTimeout(function() {
			var outsideWrapper = $('div.pull-scroll-wrapper'),
				listHeight = $('div.pull-scroll-wrapper').find('ul').outerHeight(),
				viewPortHeight = $('body.ui-mobile-viewport').outerHeight(),
				listRest = $('div.pull-scroll-wrapper').find('ul').offset().top
					- $(window).scrollTop()-43;
			if (listHeight >= viewPortHeight)
				outsideWrapper.outerHeight(viewPortHeight - 55);
			else
				outsideWrapper.outerHeight(listHeight);
	...
	initiate : function(){
		...
		setTimeout(function() {
			...
			$(function() {
			    var scrollPoint = 0;
			    outsideWrapper.scroll(function() {
